...
from QuantumGate.basicGate import basicGate

class basicOperatorGate:
    """
    基础的通用计算量子门
    """
    @staticmethod
    ...
    def classicalAddition(N):
        ...
        for i in range(N):
            circuit.append(basicOperatorGate.MAJ(), [2 * i + j for j in range(3)])

        circuit.cx(2 * N, 2 * N + 1)

        for i in range(N):
            circuit.append(basicOperatorGate.UMA(), [2 * (4 - i) + j for j in range(3)])
    ...
    def QFT_addition_circuit(N):
        ...
        for i in range(N):
            for j in range(i + 1):
                circuit.append(basicGate.R_Gate(j + 1), [N + i] + [N - i + j - 1])
    ...
    def QFT_subtraction_circuit(N):
        ...
        for i in range(N):
            for j in range(i + 2):
                circuit.append(basicGate.R_daggerGate(j + 1), [N + i + 1] + [N - 1 - i + j])

        circuit.append(basicGate.QFT_dagger(N + 1), range(N + 1))

        circuit.name = 'QFT_subtraction'
        QFT_subtraction = circuit.to_gate()

        return QFT_subtraction
    @staticmethod
    def QFT_comparator_circuit(N):
        ...
        """
        circuit = QuantumCircuit(2 * N + 1)
        circuit.append(basicOperatorGate.QFT_subtraction_circuit(N), range(2 * N + 1))
        circuit.append(basicOperatorGate.QFT_addition_circuit(N), [1 + i for i in range(2 * N)])
